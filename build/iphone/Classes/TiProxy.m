...
//Rare exceptions to indicate a bug in the _kievtours code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_kievtours = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_kievtours]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.kievtours.module"
